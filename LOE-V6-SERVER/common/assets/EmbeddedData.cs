...
namespace common
{
    public class EmbeddedData : IDisposable
    {
        ...
        {
            string response = $"[{time[1]}] [{nameof(EmbeddedData)}] Loaded {total} {mainFolder}.";
            Console.ForegroundColor = ConsoleColor.DarkGreen;
            Console.WriteLine(response);
            Console.ResetColor();
        }

        public void _(string currentFilePath, int current, int total)
        {
        ...
        public EmbeddedData(string path = "assets/xmls")
        {
            ...
            try
            {
                ...
                {
                    _(xmls[i].Split('/')[1], i, xmls.Length);
